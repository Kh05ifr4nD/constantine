...
#define DEBUG_TYPE "custom-structurizecfg"
#define dprint(s) LLVM_DEBUG(dbgs() << s << "\n")
...
static cl::opt<bool> ForceSkipUniformRegions(
  "custom-structurizecfg-skip-uniform-regions",
  cl::Hidden,
  cl::desc("Force whether the StructurizeCFG pass skips uniform regions"),
  cl::init(false));

static cl::opt<bool>
    RelaxedUniformRegions("custom-structurizecfg-relaxed-uniform-regions", cl::Hidden,
...
void StructurizeCFG::analyzeLoops(RegionNode *N) {
  if (N->isSubRegion()) {
    ...
    if (Visited.count(Exit)) {
      ...
      dprint("Set exit: " << Exit->getName().str());
      dprint(" for: " << N->getEntry()->getName().str());
    }
    ...
    for (BasicBlock *Succ : Term->successors())
      if (Visited.count(Succ)) {
        Loops[Succ] = BB;
        dprint("Set exit: " << Succ->getName().str());
        dprint(" for: " << BB->getName().str());
      }
  }
}

Value *StructurizeCFG::invertCondition(Value *Condition) {
...
void StructurizeCFG::gatherPredicates(RegionNode *N) {
  ...
  dprint("Gather preds starting from:" << *BB);
  for (BasicBlock *P : predecessors(BB)) {
    dprint("Gather preds in:" << *P);
    ...
    if (R == ParentRegion) {
      ...
      dprint("Entry: " << *Entry);
      dprint("Set to bool value true/false");
...
void StructurizeCFG::insertConditions(bool Loops) {
  ...
  for (BranchInst *Term : Conds) {
    ...
    dprint("Checking predicates in: " << (Loops ? SuccFalse->getName().str() : SuccTrue->getName().str()));
    ...
    if (ParentValue) {
      ...
      dprint("B: " << Dominator.resultIsRememberedBlock() << "" << *Parent);
...
void StructurizeCFG::addPhiValues(BasicBlock *From, BasicBlock *To) {
  for (PHINode &Phi : To->phis()) {
    ...
    dprint("Add undef phi val for:" << Phi);
  }
...
void StructurizeCFG::changeExit(RegionNode *Node, BasicBlock *NewExit,
                                bool IncludeDominator) {
  dprint("Changing exit of all nodes from " << Node->getEntry()->getName().str() << " to " << NewExit->getName().str());
  dprint("Before: " << *Node->getEntry());
  if (Node->isSubRegion()) {
    ...
    for (auto BBI = pred_begin(OldExit), E = pred_end(OldExit); BBI != E;) {
      ...
      if (!SubRegion->contains(BB))
        continue;
      dprint("Changing exit of " << BB->getName().str() << " to " << NewExit->getName().str());
...
BasicBlock *StructurizeCFG::getNextFlow(BasicBlock *Dominator) {
  ...
  dprint("getNextFlow: inserting " << Flow->getName().str() << " in region " << ParentRegion->getNameStr());
  return Flow;
}
...
BasicBlock *StructurizeCFG::needPrefix(bool NeedEmpty) {
  ...
  dprint("-> needPrefix for: " << *Entry);

  if (!PrevNode->isSubRegion()) {
    killTerminator(Entry);
    if (!NeedEmpty || Entry->getFirstInsertionPt() == Entry->end()) {
      dprint("returning Prefix: " << Entry->getName().str());
      return Entry;
    }
  }
  ...
  dprint("Inserted Prefix: " << Flow->getName().str() << " for " << Entry->getName().str());

  // and wire it up
  changeExit(PrevNode, Flow, true);
  PrevNode = ParentRegion->getBBNode(Flow);
  dprint("PrevNode: " << *PrevNode->getEntry());
  return Flow;
}
...
BasicBlock *StructurizeCFG::needPostfix(BasicBlock *Flow,
                                        bool ExitUseAllowed) {
  dprint("-> needPostfix for: " << *Flow);
  
  if (!Order.empty() || !ExitUseAllowed) {
    BasicBlock *ff = getNextFlow(Flow);
    dprint("Inserted Postfix: " << ff->getName().str() << " for " << Flow->getName().str());
    return ff;
  }
  ...
  dprint("Got Postfix: " << Exit->getName().str() << " for " << Flow->getName().str());
...
void StructurizeCFG::wireFlow(bool ExitUseAllowed,
  ...
  dprint("-> wireFlow for: " << Node->getEntry()->getName().str());

  if (isPredictableTrue(Node)) {
    dprint(" - predictable true node");
    // Just a linear flow
    if (PrevNode) {
    ...
    dprint("Adding extra prefix node: " << Flow->getName().str());

    // Insert extra postfix node (or use exit instead)
    BasicBlock *Entry = Node->getEntry();
    BasicBlock *Next = needPostfix(Flow, ExitUseAllowed);
    dprint("Adding extra postfix node: " << Next->getName().str());
    dprint(*Next);
    ...
    dprint("Branch pushed to prefix: " << *Flow);
...
void StructurizeCFG::handleLoops(bool ExitUseAllowed,
  ...
  dprint("-> handleLoops for: " << Node->getEntry()->getName().str());
  dprint("Handling loop: " << (LoopStart? LoopStart->getName().str() : "null") << " => " << (LoopEnd? LoopEnd->getName().str() : "null"));
  dprint("Node: " << *Node->getEntry());

  if (!Loops.count(LoopStart)) {
    wireFlow(ExitUseAllowed, LoopEnd);
    dprint("Loops count false");
    dprint("Node after wire flow1: " << *Node->getEntry());
    if(Node->getEntry()->getSingleSuccessor()) {
      dprint("Successor after wire flow: " << *Node->getEntry()->getSingleSuccessor());
    }
    return;
  }
  ...
  if (!isPredictableTrue(Node)) {
    ...
    // Update the predicates for the loop
    // if (!LoopPreds.count(LoopStart)) {
    //   LoopPreds[LoopStart] = LoopPreds[Node->getEntry()];
    // }
  }
  dprint("  loopStart: " << LoopStart->getName().str());

  LoopEnd = Loops[Node->getEntry()];
  wireFlow(false, LoopEnd);
  dprint("Node after wire flow2: " << *Node->getEntry());
    if(Node->getEntry()->getSingleSuccessor()) {
      dprint("Successor after wire flow: " << *Node->getEntry()->getSingleSuccessor());
    }
  ...
  dprint("Adding extra loop node");
  dprint("  now loopStart: " << LoopStart->getName().str());
  LoopEnd = needPrefix(false);
  BasicBlock *Next = needPostfix(LoopEnd, ExitUseAllowed);
  LoopConds.push_back(BranchInst::Create(Next, LoopStart,
                                        BoolUndef, LoopEnd));
  dprint("Added branch to: " << *LoopEnd);
  addPhiValues(LoopEnd, LoopStart);
  setPrevNode(Next);
  dprint("Node at end: " << *Node->getEntry());
...
void StructurizeCFG::createFlow() {
  ...
  dprint("Create flow for exit: " << ParentRegion->getNameStr());
...
bool StructurizeCFG::runOnRegion(Region *R, RGPassManager &RGM) {
  ...
  dprint("-------------- runOnRegion: " << R->getNameStr() << " --------------");
  if (SkipUniformRegions) {
    ...
    unsigned UniformMDKindID =
        R->getEntry()->getContext().getMDKindID("custom-structurizecfg.uniform");
    if (hasOnlyUniformBranches(R, UniformMDKindID)) {
...
RegisterPass<StructurizeCFG> MP("psr-custom-structurizecfg", "custom StructurizeCFG Pass");
